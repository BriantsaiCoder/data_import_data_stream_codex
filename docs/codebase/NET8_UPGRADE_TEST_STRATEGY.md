...
| **B** | **`double.TryParse` 解析翻轉**：`-1.#IND`/`1.#QNAN`/`1.#INF` 等 Windows legacy 字面量在 net462 可能 parse 成 NaN/Infinity，net8 可能 fail→0；`Infinity`/`NaN`/超界 `1E400` 行為亦不同 | 靜默（discard-vs-keep、0-vs-NaN） | [CalculateSPC.cs:48/52/65/118](../../DCT_data_import/Common/CalculateSPC.cs:65)、[FileProcess.cs:1527](../../DCT_data_import/FileAccess/FileProcess.cs:1527)、[FailPin.cs](../../DCT_data_import/ReadAndImport/FailPin.cs) |
...
廣度計劃對遷移敏感輸入用**硬編斷言**（例：「`-1.#IND`/`1.#QNAN`/blank → 0」）。但 root cause B 已驗證：**這無法從程式碼判定，是 runtime 框架差異**。若 net462 其實把 `-1.#IND` parse 成 NaN（非 0），`Assert.Equal(0, …)` 在 net462 上就是**綠燈但錯**，零防護。
...
| `DoubleToSqlString`（root cause C 的探針 seam） | 目前是 314/617 隱式 `ToString()` | 抽 `private string DoubleToSqlString(double d)` 供 golden-master——**唯一一處結構性改動，行為須保留並標註** | 中（碰高扇入 `FileProcess`，動前先 `deps-check`） |
...
P0（升級閘門 / 會悄悄改資料）
  ...
  C  ★ DoubleToSqlString golden-master（314/617，net462 基準先跑，capture）
  D  ★ CustomizeDateTimeParser 在固定 + 非invariant culture 下 capture（逼出漂移）
  B  ★ AverageOfSumSquare 含特殊浮點輸入列的 characterization（capture；NaN 會穿過 SeperatePassValue spec 比較→汙染 stdev，見下）
...
> **B→SPC 汙染鏈**：若 net462 把 `-1.#IND`→NaN，NaN 在 `SeperatePassValue` 的 `> spec_max || < spec_min` 比較**永遠 false（不被篩掉）**→ 流進 sum-of-square / stdev（[CalculateSPC.cs:83-92](../../DCT_data_import/Common/CalculateSPC.cs:83)）→ 整組 SPC 輸出變 NaN；net8 若改成 0，輸出整個不同。故 P0 的 `AverageOfSumSquare` 測試**必須含一列特殊浮點輸入**。
...
2. 依 §5 開最小 seam（`InternalsVisibleTo` + 抽 `ImportDecision` / `DoubleToSqlString`），**不碰業務邏輯**；動 `FileProcess` 前先 `deps-check`。
...
## 9) 待現場確認（agent 宣稱、未在真實 runtime 驗證）

- root cause B 的「net462 把 `-1.#IND`/`1.#QNAN`/`1.#INF` parse 成 NaN/Infinity」——**程式碼無法判定，須 net462 實跑確認**（正是 §4 capture 要捕捉的）。
- root cause C 中「`Infinity`/`NaN` 在 `zh-Hant-TW` CurrentCulture（ICU）下會本地化成非 ASCII 字面量（`∞` / `非數值`）注入 INSERT」——屬 agent 宣稱，**須 net8 + 對應 culture 實跑確認**。
- 此兩點不影響測試該不該寫（要寫），只影響期望值方向——故一律走 capture-don't-assert。
