...
[Program.Main 環境偵測 + 3 執行緒]
   ...
   -> [FileProcess.Import*：DataTable -> Dapper 參數化 INSERT(分批) -> DatabaseService.ExecuteSql]
...
| Synchronous DB wrapper | `DbApi/DatabaseService.cs` | `ExecuteSql` 包 `DBmysql.Excute_mysql_cmd`;同步模型與方法簽名一致 |
