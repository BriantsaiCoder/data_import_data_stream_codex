...
- **Phase 0 幾乎全是加法**：新增 capture 測試檔 + 文件。唯一 production 改動是 P0-1 的一行 `private`→`internal`（行為零變更，`InternalsVisibleTo` 已存在於 `AssemblyInfo.cs:22`）。
...
| Category | 測試 | CI 處置 |
|---|---|---|
| `CaptureBaseline` | P0-1 / P0-2 / P0-3 的 capture 測試（emit-only，經 `ITestOutputHelper` 印值、**不硬斷言**） | 排除綠燈門檻，由專屬 capture step（`--filter Category=CaptureBaseline --logger "console;verbosity=detailed"`）收 net462 實跑值 |
...
- B 腿（:1527）：用 culture-less `double.TryParse(string, out)`，net462 接受 Windows-CRT token `-1.#IND`/`1.#QNAN`/`1.#INF`→NaN/Inf，net8 拒絕→回 0；且受 `CurrentCulture` 影響（NLS vs ICU）。
...
**Seam（唯一 production 改動）**：`private` → `internal`。`[assembly:InternalsVisibleTo("DCT_data_import.Tests")]` **已存在** `AssemblyInfo.cs:22`，無需新增。行為零變更。
> ⚠️ critic 修正：先前草案誤稱「需加 InternalsVisibleTo」，實際已存在，本 task 唯一改動就是一行 `private`→`internal`。

**步驟**：
1. `FileProcess.cs:1515` 的 `ValidateAndConvertStatisticValue` 簽章 `private` → `internal`（其他不動）。
2. 新增 `DCT_data_import.Tests/ValidateAndConvertStatisticValueTests.cs`：
...
**為什麼**：[DBmysql.cs:147](../../DCT_data_import/MySQL_api/DBmysql.cs#L147) `dateTime.ToString("yyyy-MM-dd HH:mm:ss")` 進 `JObject`。值來自 MySql.Data 查詢結果**非字串 parse**，故無上游 parse 漂移；format 本身明確數字、`HH` 24h 無截斷——**Tier-1 三個 datetime 目標中最低漂移**。真正 library 依賴的是 DB→DateTime materialization（MySql.Data 9.4.0），那是 integration scope。
...
**上線後 manual smoke 清單**（寫進文件，Phase 1 cutover 用）：
- [ ] 各 importer 啟動不因 empty-NIC 丟 `IndexOutOfRange`。
- [ ] MySql.Data 9.4.0 從真實 DB 撈出的 DateTime 字串格式與 net462 一致（抽樣比對）。
- [ ] FTP 目錄列表解析（檔名 pattern match）在 net8 結果與 net462 一致。
...
1. ✅ P0-1 / P0-2 / P0-3 的 `CaptureBaseline` 測試已寫，CI windows-latest capture step **跑出 net462 實跑基準並存成 log artifact**（權威基準，非手寫）。
