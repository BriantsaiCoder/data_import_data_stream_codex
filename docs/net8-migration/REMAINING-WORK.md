...
- [ ] **S2（HIGH，唯一待動主線；A/PR-1 已完成）**：SQL 全字串串接零參數化（`FileProcess`/`DbAccess`/`TsmcIeda`）→ 改參數化（Dapper 具名參數）。已完成 `DbAccess` / `TsmcIeda` / identifier chokepoint；仍待 A/PR-2 單獨處理 `FileProcess.Import*` 批次 INSERT values。**高扇入共用檔，動前先 `deps-check`**。完整計畫見 [S2-SQL-PARAMETERIZATION-PLAN.md](S2-SQL-PARAMETERIZATION-PLAN.md)、交接見 [HANDOFF-S2.md](HANDOFF-S2.md)。
