...
- (b) net8 **自包含發佈**使回切不依賴目標機是否裝 .NET 8 runtime，且與 net462 並存無衝突。
- (c) 回切點**預先寫成 runbook + 演練一次**，不要等出事才想流程。

> ⚠️ **乾演練（dry-run rehearsal）待 Windows 環境執行（A0 未竟項）**：下列 L2/L3 步驟已寫成可執行指令，但「真的停一次 process、切一次資料夾、復原一次 DB」必須在 Windows + 真 MySQL 環境演練，無法在 macOS 完成。本機僅驗證發佈指令可還原/編譯。

#### L2 可執行步驟（產物資料夾切換 `app_net8\` ↔ `app_net462\`）

**前置：產出兩版產物（self-contained，可重現）**

```powershell
# net8 自包含產物(指向 win-x64,不依賴目標機是否裝 .NET 8 runtime)
dotnet publish DCT_data_import\DCT_data_import.csproj -f net8.0-windows -r win-x64 --self-contained -c Release
# 產物在 DCT_data_import\bin\Release\net8.0-windows\win-x64\publish\,整包複製到 app_net8\

# net462 fallback 產物(framework-dependent;目標機已裝 .NET Framework 4.6.2)
msbuild DCT_data_import.sln /p:Configuration=Release /p:TargetFramework=net462
# 產物在 DCT_data_import\bin\Release\net462\,整包複製到 app_net462\
```

> csproj 已設 `<RuntimeIdentifiers>win-x64</RuntimeIdentifiers>`（僅 net8.0-windows TFM、複數，供 restore 提示）；未設 `<SelfContained>`／單數 `<RuntimeIdentifier>`，故 `dotnet build` 行為不變、self-contained 只在上面 publish 指令顯式帶旗標時觸發。

**佈署佈局（兩版並存，同一份 App.config）**

```
D:\DCT\
├─ app_net8\      ← net8 self-contained publish 整包
├─ app_net462\    ← net462 framework-dependent 產物
└─ App.config     ← 同一份(帳密不變);兩產物各自的 *.dll.config 由佈署時複製此檔覆蓋
```

**正向切到 net8（cutover step 6）**

```powershell
# 1. 停目前 net462 process(以實際 process / 排程器名稱為準,此處為佔位)
Stop-Process -Name DCT_data_import -Force   # 或停對應 Windows 排程/服務
# 2. 確認 app_net8\ 已佈署且 DryRun=false(production 一律 false,見 P1-7b)
#    -> 檢查 app_net8\DCT_data_import.dll.config 的 <add key="DryRun" value="false" />
# 3. 啟動 net8 產物
Start-Process D:\DCT\app_net8\DCT_data_import.exe
```

**回切到 net462（L2 rollback；net8 異常時）**

```powershell
# 1. 停 net8 process
Stop-Process -Name DCT_data_import -Force
# 2. 啟動 net462 fallback(同一份 App.config,DryRun 一律 false)
Start-Process D:\DCT\app_net462\DCT_data_import.exe
# 3. 確認首幾輪輪詢恢復寫入(觀察 data_import_logs / check_logs)
```

> **安全**：runbook 只記資料夾路徑與 process 名，**不記任何 App.config 帳密值**（CLAUDE.md Hard Rule / S1/S3）。切換不動 App.config，帳密不變。
...
#### L3 可執行步驟（DB snapshot / binlog 還原）

> ⚠️ **乾演練待 Windows + 真 MySQL 環境執行（A0 未竟項）**：下列指令以「在能連到 production MySQL 的維運機」執行為前提，本機（macOS、無真 MySQL）無法演練。實際帳密／host／DB 名走 App.config，**runbook 不寫值**（下方一律佔位）。

**(1) cutover 前：建立 snapshot baseline（step 6 切 net8 之前）**

```bash
# 邏輯快照(可直接 reload):整庫或至少受 ETL 寫入的表(db_key / db_key_ui_status / lots_* / *_anomaly* / ieda_* / recovery_rate ...)
mysqldump --single-transaction --routines --triggers \
  -h <DB_HOST> -u <DB_USER> -p <DB_NAME> > dct_pre_cutover_$(date +%Y%m%d_%H%M%S).sql
```

**(2) cutover 期：開啟 / 確認 binlog 保留窗（時間點還原的依據）**

```sql
-- 確認 binlog 已開、格式為 ROW(可精確回放單列)
SHOW VARIABLES LIKE 'log_bin';                 -- 應為 ON
SHOW VARIABLES LIKE 'binlog_format';           -- 建議 ROW
SHOW VARIABLES LIKE 'binlog_expire_logs_seconds';  -- 確保保留窗 ≥ 觀察期(≥1 完整營運週期)
-- 記下切 net8 當下的座標,作為 PITR 起點
SHOW MASTER STATUS;                            -- 記 File + Position(或 GTID)
```

**(3) net8 異常且已污染資料 → 還原該段**

選項 A — **整庫回滾到 cutover 前 snapshot**（最簡單、最暴力；會丟掉 cutover 後所有寫入，僅適用觀察期內、可接受重跑該段資料）：

```bash
mysql -h <DB_HOST> -u <DB_USER> -p <DB_NAME> < dct_pre_cutover_<timestamp>.sql
```

選項 B — **PITR（point-in-time recovery）回到污染發生前**（保留 cutover 後到污染點之間的正確寫入）：

```bash
# 1. 還原 cutover 前 snapshot 當基底
mysql -h <DB_HOST> -u <DB_USER> -p <DB_NAME> < dct_pre_cutover_<timestamp>.sql
# 2. 用 binlog 重放 snapshot 之後到「污染點前一刻」的交易(--stop-datetime 設為污染發生前)
mysqlbinlog --start-position=<step2 記下的 Position> \
  --stop-datetime="YYYY-MM-DD HH:MM:SS" <binlog files...> \
  | mysql -h <DB_HOST> -u <DB_USER> -p <DB_NAME>
```

> **判斷用哪個選項**：觀察期內、cutover 後寫入量小且可重跑 → 選項 A。已累積大量正確寫入、只要剔除污染段 → 選項 B（PITR）。**還原前務必先停所有寫入 process**（net8 與 net462 都停），避免邊還原邊被覆寫。
> **安全**：上述所有指令的 `-p` 不帶值（互動式輸入密碼），帳密不寫進 runbook / shell history（CLAUDE.md Hard Rule）。
...
| 5 | 平行運行影子模式（dry-run，**開關由 P1-7b 先落地**，設 `DryRun=true`）：先在影子環境的獨立 schema 跑 [`DCT_data_import/sql/dct.sql`](../../DCT_data_import/sql/dct.sql) 建表（`db_key`／`db_key_ui_status`／`lots_*`／`*_anomaly*`／`ieda_*`／`recovery_rate` 等 DDL），影子佇列指獨立工作目錄，餵真實 FTP CSV，與 net462 production 同期逐筆比對 db_key 決策/匯入結果/時間字串，跑滿 ≥1 完整營運週期（各 importer × 各 check_status bit × 輪詢） |
